...
Not many tools exist for generating JWKs files, I found that Bespoke Engineering's open source JWK generator works the best. It comes in an https://mkjwk.org/[online version] and a https://github.com/bspk/json-web-key-generator[CLI]. *For security purposes it is recommended to use the CLI as processing occurs on your computer instead of an untrusted external server which could potentially store keys.*
